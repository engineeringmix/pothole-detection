...
def process_image(image, model, confidence):
    ...
    results = model(opencv_image, conf=confidence, device=device)
...
def main():
    ...
    with col2:
        if 'detections' in locals() and uploaded_file is not None:
            ...
            # Priority messages
            if high_count > 0:
                st.error(f"⚠️ **HIGH PRIORITY**: {high_count} severe pothole(s) detected! Immediate maintenance recommended.")
            elif medium_count > 0:
                st.warning(f"⚠️ **MEDIUM PRIORITY**: {medium_count} moderate pothole(s) detected. Schedule maintenance within 2-4 weeks.")
            elif low_count > 0:
                st.info(f"ℹ️ **LOW PRIORITY**: {low_count} minor pothole(s) detected. Monitor and schedule routine maintenance.")
            else:
