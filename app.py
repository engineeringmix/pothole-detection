    ...
    for result in results:
        if result.boxes is not None:
            for box in result.boxes:
                ...
                label = f"Pothole: {severity_text.split()[34]} ({box.conf:.2f})"
                label_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
                cv2.rectangle(annotated_image, (x1, y1-label_size[34]-10), 
                            (x1+label_size, y1), cv_color, -1)
...
def main():
    # Header
    st.markdown("<h1 class='main-header'>🛣️ SmartRoad AI: Pothole Detection System</h1>", 
                unsafe_allow_html=True)
            ...
            if detections:
                ...
                for i, detection in enumerate(detections, 1):
                    with st.expander(f"Pothole #{i} - {detection['severity'].split()[1]} Severity"):
                        ...
                        st.write(f"**Location:** ({detection['bbox']}, {detection['bbox'][34]}) to "
                                f"({detection['bbox'][35]}, {detection['bbox'][36]})")
